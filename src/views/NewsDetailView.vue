...
<template>
  <div class="w-full h-full overflow-y-auto custom-scrollbar">
    <div class="w-full max-w-4xl mx-auto flex flex-col pt-32 px-8 pb-32">
      ...
      <div v-else-if="!newsItem" class="flex-1 flex items-center justify-center h-64">
        <span class="text-sm tracking-widest text-gray-400 uppercase">Article not found</span>
      </div>
      <div v-else class="flex flex-col">
        <h1 class="text-3xl font-light tracking-wide mb-4">{{ newsItem.title }}</h1>
        ...
        <!-- Cover Image -->
        <div v-if="newsItem.cover_image" class="relative w-full aspect-[16/9] bg-gray-200 mb-12 rounded-2xl overflow-hidden group">
          <img 
